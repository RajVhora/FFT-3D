...
void write_ppfiles(int n_x, int n_y, int n_z, double delta_t1, int time_steps1, 
...
fprintf(fpp,"int n_x,int n_y, int n_z,int N);\n");
fprintf(fpp,"\n");
...
fprintf(fpp,"if( (fp=fopen(\"%s\",\"r\")) == NULL){\n",name);
fprintf(fpp,"printf(\"Generating the psfile %s\\n\");\n",name);
fprintf(fpp,"generate_psfile(\"%s\",\"%s\",%d,%d,%d);\n",NAME,name,n_x,n_y,n_x);
fprintf(fpp,"}\n");
}
