...
double eps0_11c,eps0_22c,eps0_33c,eps0_12c,eps0_13c,eps0_23c;
//double eps0_11e,eps0_22e,eps0_33e,eps0_12e,eps0_13e,eps0_23e;
...
double Ceff[4][4][4][4], DeltaC[4][4][4][4];
...
fscanf(fpr,"%le",&eps0_33c);
fscanf(fpr,"%le",&eps0_12c);
fscanf(fpr,"%le",&eps0_13c);
fscanf(fpr,"%le",&eps0_23c);
//fscanf(fpr,"%le",&eps0_11e);
//fscanf(fpr,"%le",&eps0_22e);
//fscanf(fpr,"%le",&eps0_33e);
//fscanf(fpr,"%le",&eps0_12e);
//fscanf(fpr,"%le",&eps0_13e);
//fscanf(fpr,"%le",&eps0_23e);
fscanf(fpr,"%le",&c11m);
fscanf(fpr,"%le",&c12m);
fscanf(fpr,"%le",&c44m);
fscanf(fpr,"%le",&c11p);
...
fscanf(fpr,"%le",&sig_app[3][3]);
fscanf(fpr,"%le",&sig_app[1][2]);
fscanf(fpr,"%le",&sig_app[1][3]);
fscanf(fpr,"%le",&sig_app[2][3]);
fscanf(fpr,"%d",&MAXITR);
...
fscanf(fpr,"%d%d%d%le%le%le",&n_x,&n_y,&n_z,&delta_x,&delta_y,&delta_z);
...
fscanf(fpr,"%le%le%le%le%le%le",&eps0_11c,&eps0_22c,&eps0_33c,&eps0_12c,&eps0_13c,&eps0_23c);
//fscanf(fpr,"%le%le%le%le%le%le",&eps0_11e,&eps0_22e,&eps0_33e,&eps0_12e,&eps0_13e,&eps0_23e);
fscanf(fpr,"%le%le%le",&c11m,&c12m,&c44m);
fscanf(fpr,"%le%le%le",&c11p,&c12p,&c44p);
fclose(fpr);

sig_app[2][1] = sig_app[1][2];
sig_app[3][1] = sig_app[1][3];
sig_app[3][2] = sig_app[2][3];
...
fscanf(fpr,"%le%le%le%le%le%le",&sig_app[1][1],&sig_app[2][2],&sig_app[3][3],&sig_app[1][2],&sig_app[1][3],&sig_app[2][3]);
fclose(fpr);
...
c = dvector(1,n_x*n_y*n_z);
comp = fftw_malloc(n_x*n_y*n_z*sizeof(fftw_complex));

//e = dvector(1,n_x*n_y*n_z);
//eta = fftw_malloc(n_x*n_y*n_z*sizeof(fftw_complex));
...
tmp = fread(&c[1],sizeof(double),(size_t)n_x*n_y*n_z,fpr);
fclose(fpr);
}
else if(HOWTO == 0){
/** Read compsotion **/
...
tmp = fread(&e[1],sizeof(double),(size_t)n_x*n_y*n_z,fpr);
fclose(fpr);*/
...
fpw = fopen("../output/ini_comp_time0.data","w");
for(i=0; i < n_x; ++i){
for(j=0; j < n_y; ++j){
for(k=0; k < n_z; ++k){
J = k + n_z*(j+n_y*i);
...
for(k=0; k < n_z; ++k){
J = k + n_z*(j+n_y*i);
fprintf(fpw,"%d %d %d %le\n",i,j,k,__real__ comp[J]);
}}}
fclose(fpw);
...
sigma_Tc = dmatrix(1,3,1,3);  
epsilon_Tc = dmatrix(1,3,1,3);
//sigma_Te = dmatrix(1,3,1,3);
//epsilon_Te = dmatrix(1,3,1,3);
calculate_sigma_T(Ceff,eps0_11c,eps0_22c,eps0_33c,eps0_12c,eps0_13c,eps0_23c,sigma_Tc,epsilon_Tc);
//calculate_sigma_T(Ceff,eps0_11e,eps0_22e,eps0_33e,eps0_12e,eps0_13e,eps0_23e,sigma_Te,epsilon_Te);

printf("Calculated eigen strain and stress\n");

system("rm -rf ../output/SG_Analysis"); 
if(sig_app[1][1] != 0.0 || sig_app[2][2] != 0.0 || sig_app[3][3] != 0.0){
...
system("rm -rf ../output/postprocessor/data/src/datamakerc.c");
system("rm -rf ../output/postprocessor/data/src/datamakere.c");
write_ppfiles(n_x,n_y,n_z,delta_t1,time_steps1,t0,delta_t2,time_steps2,t1,
...
evolve_diffusional(n_x,n_y,n_z,delta_x,delta_y,delta_z,kappac,A,delt,tsteps,0,
...
for(k=0; k < n_z; ++k){
J = k + n_z*(j+n_y*i);
fprintf(fpr,"%d %d %d %le\n",i,j,k,__real__ (comp[J]));
if(__real__ comp[J]>=1.5)
printf("1 %d %d %d %le\n",i,j,k,__real__ comp[J]);
}}}
fclose(fpr);
...
fprintf(fpw,"n_x: %d n_y: %d n_z: %d\n",n_x,n_y,n_z);
fprintf(fpw,"delta_x: %le delta_y: %le delta_z: %le\n",delta_x,delta_y,delta_z);
...
evolve_diffusional(n_x,n_y,n_z,delta_x,delta_y,delta_z,kappac,A,delta_t1,time_steps1,t0,
comp,ave_comp,STEPS,plan_indicator,SNA,NOISEADDER,noise_str);
evolve_diffusional(n_x,n_y,n_z,delta_x,delta_y,delta_z,kappac,A,delta_t2,time_steps2,t1,
comp,ave_comp,STEPS,plan_indicator,SNA,NOISEADDER,noise_str);
evolve_diffusional(n_x,n_y,n_z,delta_x,delta_y,delta_z,kappac,A,delta_t3,time_steps3,t2,
comp,ave_comp,STEPS,plan_indicator,SNA,NOISEADDER,noise_str); 
}
...
fprintf(fpw,"eps0_33c: %le\t",eps0_33c);
fprintf(fpw,"eps0_12c: %le\n",eps0_12c);
fprintf(fpw,"eps0_13c: %le\n",eps0_13c);
fprintf(fpw,"eps0_23c: %le\n",eps0_23c);
//fprintf(fpw,"eps0_11e: %le\t",eps0_11e);
//fprintf(fpw,"eps0_22e: %le\t",eps0_22e);
//fprintf(fpw,"eps0_33e: %le\t",eps0_33e);
//fprintf(fpw,"eps0_12e: %le\n",eps0_12e);
//fprintf(fpw,"eps0_13e: %le\n",eps0_13e);
//fprintf(fpw,"eps0_23e: %le\n",eps0_23e);
...
fprintf(fpw,"sig_app[3][3]: %le\n",sig_app[3][3]);
fprintf(fpw,"sig_app[1][2]: %le\t",sig_app[1][2]);
fprintf(fpw,"sig_app[1][3]: %le\t",sig_app[1][3]);
fprintf(fpw,"sig_app[2][3]: %le\t",sig_app[2][3]);
...
for(j=0; j < n_y; ++j){
for(k=0; k < n_z; ++k){
if(__real__ comp[k + n_z*(j+n_y*i)]>=1.5)
printf("2 %d %d %d %le\n",i,j,k,__real__ comp[k + n_z*(j+n_y*i)]);
}}}
//

if(INDICATOR==0){ 
evolve_inhom_constrained(n_x,n_y,n_z,delta_x,delta_y,delta_z,kappac,A,delta_t1,
...
evolve_inhom_constrained(n_x,n_y,n_z,delta_x,delta_y,delta_z,kappac,A,delta_t2,
time_steps2,t1,comp,Ceff,DeltaC,sigma_Tc,epsilon_Tc,ave_comp,sig_app,STEPS,
MAXITR,MAXERR,n_alpha,n_beta,alpha,beta,alpha_prime,beta_prime,plan_indicator,
SNA,NOISEADDER,noise_str);
evolve_inhom_constrained(n_x,n_y,n_z,delta_x,delta_y,delta_z,kappac,A,delta_t3,
...
fftw_free(comp);
free_dmatrix(sig_app,1,3,1,3);
free_dmatrix(sigma_Tc,1,3,1,3);
free_dmatrix(epsilon_Tc,1,3,1,3);
free_dvector(c,1,n_x*n_y*n_z);
