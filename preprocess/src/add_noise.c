...
void add_noise(int n_x, int n_y, int n_z, double *comp, 
...
temp = (double *) malloc((size_t) n_x*n_y*n_z* sizeof(double));
mean_noise = 0.0;
for(i1=0; i1<n_x; ++i1){
for(i2=0; i2<n_y; ++i2){
for(i3=0; i3<n_z; ++i3){
...
mean_noise = mean_noise/(n_x*n_y*n_z);
...
for(i3=0; i3<n_z; ++i3){
J = i3 + n_z*(i2+n_y*i1);
comp[J] = comp[J] - mean_noise; 
average_composition = average_composition + comp[J];
}}}
average_composition = average_composition/(n_x*n_y*n_z);

fp = fopen("output/ini_comp_time0.data","w");
for(i1=0; i1 < n_x; ++i1){
for(i2=0; i2 < n_y; ++i2){
for(i3=0; i3 < n_z; ++i3){
J = i3 + n_z*(i2+n_y*i1);
...
tmp = fwrite(&comp[0],sizeof(double),(size_t)n_x*n_y*n_z,fp);
fclose(fp);
