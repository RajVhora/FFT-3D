...
void generate_initial_composition_profile(int n_x, int n_y, int n_z,
double *comp){
...
spinodal(n_x,n_y,n_z,ave_comp,comp);
}
...
fscanf(fp,"%d%d",&SNA,&NOISEADDER);
circular(n_x,n_y,n_z,R,comp_precipitate,comp_matrix,comp);
}
...
fscanf(fp,"%d%d",&SNA,&NOISEADDER);
thinfilm(n_x,n_y,n_z,h,comp_precipitate,comp_matrix,comp);
}
...
else if(strncmp(initial_profile,ip7,5)==0){
fscanf(fp,"%c",&junk);
fscanf(fp,"%d%le%le%le",
&R,&comp_precipitate,&comp_matrix,&noise_strength);
fscanf(fp,"%d%d",&SNA,&NOISEADDER);
oblate(n_x,n_y,n_z,R,comp_precipitate,comp_matrix,comp);
}
else{
...
fprintf(fp,"Oblate spherois precipitates of radius %d\n",R);
fprintf(fp,"c_p = %le; c_m = %le\n",
comp_precipitate,comp_matrix);
}
fclose(fp);
...
fclose(fp);*/
        ...
        fp = fopen("../output/init_prof.vtk","w");   
	fprintf(fp, "# vtk DataFile Version 4.2.0\n");
	fprintf(fp, "Order Parameter data\n");
	fprintf(fp, "ASCII\n");
	...
	fprintf(fp, "SCALARS order_parameter double\n");
